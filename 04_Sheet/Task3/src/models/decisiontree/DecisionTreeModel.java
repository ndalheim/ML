...
public class DecisionTreeModel {
    ...
    public void trainModel(DataSet dataset,
        ...
        Node root = new Node(null, null, validRows);
        trainModelOnSubset(dataset, remainingAttributes,
                root, classAttribute, 0);
        setRoot(root);
    }


    /**
     * Submethod of the trainModel-method to create all the childnodes of models.decisiontree.DecisionTreeModel-root node
     * on the remaining attributes and remaining valid rows.
     *
     * @param dataset of the arff file
     * @param remainingAttributes are all attributes which are left
     * @param newNode is the new generated models.decisiontree.Node in the decision tree
     * @param classAttribute is the attribute which use to test your decision tree
    ...
    protected void trainModelOnSubset(DataSet dataset,
        ...
        double entropy = entropyOnSubset(dataset, newNode.getValidRows(), classAttribute);
        if (entropy == 0.0) {
            int index = Utils.getIndexOfFirstOne(newNode.getValidRows());
            if (index >= 0) {
                newNode.setResultValue(dataset.getEntryInDataSet(classAttribute, index));
            } else {

                double[] probs = dataset.computeProbabilities(newNode.getParent().getValidRows(), classAttribute);
                int argMaxIndex = Utils.argMax(probs);
                newNode.setResultValue(classAttribute.getValues().get(argMaxIndex));
                return;
            }
            return;
        }
        if(depth == maxDepth || remainingAttributes.size() == 0){

            double[] probs = dataset.computeProbabilities(newNode.getValidRows(), classAttribute);
        ...
        for (Attribute attribute : remainingAttributes) {
            double gain = informationGain(dataset, newNode.getValidRows(), classAttribute, attribute);
            if (gain > maxGain) {
                maxGain = gain;
                maxAtt = attribute;
            }
        ...
        ArrayList<int[]> validRowsForChilds = dataset.filterForAttributeValues(newNode.getValidRows(), maxAtt);
        for (int i = 0; i < maxAtt.getValues().size(); i++) {

            String value = maxAtt.getValues().get(i);

            Node child = new Node(newNode, value, validRowsForChilds.get(i));
